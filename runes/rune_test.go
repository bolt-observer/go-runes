...
func TestFromString(t *testing.T) {
	for _, str := range []string{"374708fff7719dd5979ec875d56cd2286f6d3cf7ec317a3b25632aab28ec37bb:", "6035731a2cbb022cbeb67645aa0f8a26653d8cc454e0e087d4d19d282b8da4bd:=1",
		...
		assert.NoError(t, err, fmt.Sprintf("Failed %s", str))
		assert.Equal(t, str, r.String(), fmt.Sprintf("Failed %s", str))
	}
}
...
func TestFromBase64(t *testing.T) {
	for _, str := range []string{"N0cI__dxndWXnsh11WzSKG9tPPfsMXo7JWMqqyjsN7s", "YDVzGiy7Aiy-tnZFqg-KJmU9jMRU4OCH1NGdKCuNpL09MQ",
		...
		assert.NoError(t, err, fmt.Sprintf("Failed %s", str))
		assert.Equal(t, str, r.ToBase64Internal(true), fmt.Sprintf("Failed %s", str))
	}
}
...
func TestUnparsable(t *testing.T) {
	...
	_, err := FromBase64("tU-RL")
	assert.ErrorIs(t, err, ErrInvalidRune)
	...
	_, err = FromString("N0cI__dxndWXnsh11WzSKG9tPPfsMXo7JWMqqyjsN7s")
	assert.ErrorIs(t, err, ErrInvalidRune)

	_, err = FromString("1edf4068e2b0b1e4e075e66751c2d3f5c9fc4515d114f875e6dc6e3e6704efa9:")
	assert.NoError(t, err)
